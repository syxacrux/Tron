...
class Tree
{
	 ...
	 * @param $list array 要转换的数据集
	 * @param string $pk string ID标记字段
	 * @param string $pid string parent标记字段
	 * @param string $child string 子代key名称
	 * @param int $root string 返回的根节点ID
	 * @param bool $strict string 默认严格模式
	 * @param array $filter array 要剔除掉的字段
	 * @return array
	 */
	...
	/**
	 * 对查询结果集进行排序
	 * @param $list array 查询结果
	 * @param $field string 排序的字段名
	 * @param string $order array 排序类型 asc正向排序 desc逆向排序 nat自然排序
	 * @return array|bool
	 */
	public function list_sort_by($list, $field, $order = 'asc')
	{
		if (is_array($list)) {
